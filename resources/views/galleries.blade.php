        ...
        <div class="relative mx-auto max-w-7xl">
            <div class="container px-6 py-20 mx-auto md:text-center md:px-4">
                <h1
                    ...
                    <span class="block">{{ $profile->name }}</span></h1>
                <div class="grid grid-cols-2 gap-4 mt-8 md:grid-cols-3 lg:grid-cols-4 gallery">
                    @foreach($profile->getMedia('gallery') as $index => $media)
                        <div
                            class="overflow-hidden rounded-lg shadow-lg transform transition duration-500 hover:scale-105 gallery-item {{ $index % 4 == 0 ? 'wide' : '' }}">
                            <img src="{{ $media->getUrl() }}" alt="Gallery Image"
                                class="w-full h-full object-cover cursor-pointer glightbox"
