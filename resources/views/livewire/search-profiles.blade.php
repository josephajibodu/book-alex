    ...
    <div class="max-w-2xl mx-auto">
        <div class="relative flex items-center overflow-hidden text-left border border-gray-700 rounded-md">
            ...
            <span class="relative top-0 right-0 block">
                <button type="button" wire:click="search" class="inline-flex items-center w-32 h-12 px-8 text-base font-bold leading-6 text-white transition duration-150 ease-in-out bg-pink-500 border border-transparent hover:bg-pink-400 focus:outline-none active:bg-pink-400" data-primary="pink-500">
                    Search
                </button>
            </span>
        </div>
                        ...
                        @foreach($results as $profile)
                            <a href="{{ route('profile', $profile->slug) }}" class="block group" style="width: 320px;">
                                <div class="relative overflow-hidden rounded-lg shadow-lg bg-black">
                                    <img class="object-cover w-full h-80 transition-transform duration-300 group-hover:scale-105" src="{{ $profile->getFirstMediaUrl('featured') }}" alt="{{ $profile->name }}">
